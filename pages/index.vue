     ...
     <div class="dark:text-white px-5 h-full overflow-hidden">
          <div class="grid justify-items-center content-center h-full text-center gap-5">
               ...
               <div v-for="(item, index) in totalCats" :key="index">
                    <img v-if="item.show" class="w-12 z-20 absolute bottom-0 animate-float-balloon transform transition duration-500 hover:scale-110" :style="`left: ${item.position}vw`" src="/assets/img/cat-baloon.webp" @click="caughtCat(index)" />
               </div>
               <p v-if="started && !allTrapped">Atrapalos!</p>
               <p v-if="totalCats.some((c) => c.caught)">{{ totalCats.filter((c) => c.caught).length }} / {{ totalCats.length }} atrapados</p>
